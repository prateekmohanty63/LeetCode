...
public:
    ...
    int maxPoints(vector<int>&nums,int i,unordered_map<int,int>&mp,unordered_map<int,int>&freqMap,vector<int>&dp)
    {
        // base case
        if(i<0)return 0;
        ...
        if(mp.find(val-1)!=mp.end())
        {
            ...
            return dp[i]=max(val*freqMap[val]+maxPoints(nums,indToGo-1,mp,freqMap,dp),maxPoints(nums,i-1,mp,freqMap,dp));
        }
        else{
            return dp[i]=max(val*freqMap[val]+maxPoints(nums,mp[val]-1,mp,freqMap,dp),maxPoints(nums,i-1,mp,freqMap,dp));
        }
    ...
    int deleteAndEarn(vector<int>& nums) {
        ...
        return maxPoints(nums,n-1,indMap,freqMap,dp);
    }
};
